...
export function fmtPurchaseEmail({ email, products, total, sessionId, tier, attribution }) {
 ...
 const attrText = formatAttributionText(attribution);
 const attrHtml = formatAttributionHtml(attribution);
...
<html><body style="font-family:'DM Sans',Arial,sans-serif;color:#111;background:#fafafa;padding:24px;">
 <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:14px;padding:28px;box-shadow:0 1px 3px rgba(16,24,40,.08);">
  ...
  <table style="width:100%;border-collapse:collapse;margin-bottom:18px;">
   ...
   <tr><td style="padding:8px 0;border-bottom:1px solid #e5e5e5;color:#475467;font-size:13px;vertical-align:top;">Source</td><td style="padding:8px 0;border-bottom:1px solid #e5e5e5;">${attrHtml}</td></tr>
...
function formatAttributionText(attribution) {
 if (!attribution || typeof attribution !== 'object') return '(direct, no UTMs)';
 const parts = ATTRIBUTION_FIELDS
  .filter(([k]) => typeof attribution[k] === 'string' && attribution[k].length)
  .map(([k, label]) => `${label}=${attribution[k]}`);
 return parts.length ? parts.join(' / ') : '(direct, no UTMs)';
}

function formatAttributionHtml(attribution) {
 if (!attribution || typeof attribution !== 'object') {
  return '<span style="color:#9ca3af;">(direct, no UTMs)</span>';
 }
 const rows = ATTRIBUTION_FIELDS
  .filter(([k]) => typeof attribution[k] === 'string' && attribution[k].length)
  .map(([k, label]) => `<div style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;line-height:1.5;"><span style="color:#9ca3af;">${escapeHtml(label)}</span> = ${escapeHtml(attribution[k])}</div>`);
 return rows.length ? rows.join('') : '<span style="color:#9ca3af;">(direct, no UTMs)</span>';
}
